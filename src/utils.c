...
void pw_peer_shuffle_team_data() {
    ...
    for(size_t i = 9; i != 0; i--) {
        pw_eeprom_read(PW_EEPROM_ADDR_MET_PEER_DATA + (i-1)*PW_EEPROM_SIZE_CURRENT_PEER_TEAM_DATA, eeprom_buf, PW_EEPROM_SIZE_CURRENT_PEER_TEAM_DATA);
        pw_eeprom_write(PW_EEPROM_ADDR_MET_PEER_DATA + i*PW_EEPROM_SIZE_CURRENT_PEER_TEAM_DATA, eeprom_buf, PW_EEPROM_SIZE_CURRENT_PEER_TEAM_DATA);
    }
