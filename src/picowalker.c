...
void pw_normal_loop() {
    ...
    if (!is_in_time_sensitive_state(current_state->sid)) {
        walker_timings.now = pw_time_get_us();
        td = (walker_timings.prev_accel_check > walker_timings.now)
                 ? (walker_timings.prev_accel_check - walker_timings.now)
                 : (walker_timings.now - walker_timings.prev_accel_check);
    ...
    if (pending_state->sid != current_state->sid) {
        ...
        *pending_state = (pw_state_t){0};  // clang-format why
    ...
    walker_timings.now = pw_time_get_us();
    td = (walker_timings.prev_screen_redraw > walker_timings.now)
             ? (walker_timings.prev_screen_redraw - walker_timings.now)
             : (walker_timings.now - walker_timings.prev_screen_redraw);
        ...
        if (power_context.show_battery_low_icon) {
            if ((screen_flags.frame / 2) == 0) {
                pw_screen_draw_from_eeprom(
                    0, 0, 8, 8, PW_EEPROM_ADDR_IMG_LOW_BATTERY, PW_EEPROM_SIZE_IMG_LOW_BATTERY, true);
            } else {
