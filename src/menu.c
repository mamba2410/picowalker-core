...
static uint8_t const MENU_COSTS[] = {10, 3, 0, 0, 0, 0};
...
static void menu_draw_bottom_numbers(pw_state_t *s, const screen_flags_t *sf) {
    ...
    if (MENU_COSTS[s->menu.cursor] != 0) {
        ...
        pw_screen_draw_from_eeprom(
            24, PW_SCREEN_HEIGHT - 16, 16, 16, PW_EEPROM_ADDR_IMG_WATTS, PW_EEPROM_SIZE_IMG_WATTS, false);
        pw_screen_draw_from_eeprom(
            40, PW_SCREEN_HEIGHT - 16, 8, 16, PW_EEPROM_ADDR_IMG_CHAR_SLASH, PW_EEPROM_SIZE_IMG_CHAR, false);
    } else {
        pw_screen_clear_area(0, PW_SCREEN_HEIGHT - 16, PW_SCREEN_WIDTH / 2, 16);
    }

    pw_screen_draw_from_eeprom(
        PW_SCREEN_WIDTH - 16, PW_SCREEN_HEIGHT - 16, 16, 16, PW_EEPROM_ADDR_IMG_WATTS, PW_EEPROM_SIZE_IMG_WATTS, false);
    ...
    pw_screen_clear_area(16, PW_SCREEN_HEIGHT - 16, 8, 16);
}
...
static void menu_clear_draw_cursor(pw_state_t *s, const screen_flags_t *sf) {
    for (int i = 0; i < MENU_SIZE; i++) {
        if (s->menu.cursor == i) {
            ...
            pw_screen_draw_from_eeprom(4 + i * 16, CURSOR_Y_VALUES[i] - 8, 8, 8, addr, PW_EEPROM_SIZE_IMG_ARROW, true);
        } else {
...
void pw_menu_init_display(pw_state_t *s, const screen_flags_t *sf) {
    pw_screen_draw_from_eeprom(8, 0, 80, 16, MENU_TITLES[s->menu.cursor], PW_EEPROM_SIZE_IMG_MENU_TITLE_CONNECT, false);
    pw_screen_draw_from_eeprom(
    ...
    for (int i = 0; i < MENU_SIZE; i++) {
        pw_screen_draw_from_eeprom(
            i * 16, CURSOR_Y_VALUES[i], 16, 16, MENU_ICONS[i], PW_EEPROM_SIZE_IMG_MENU_ICON_CONNECT, true);
    }
...
void pw_menu_update_display(pw_state_t *s, const screen_flags_t *sf) {
    switch (s->menu.substate) {
        case MS_NORMAL: {
            ...
            // redraw title
            pw_screen_draw_from_eeprom(
                8, 0, 80, 16, MENU_TITLES[s->menu.cursor], PW_EEPROM_SIZE_IMG_MENU_TITLE_CONNECT, false);
        ...
        case MS_MESSAGE: {
            ...
            pw_img_t img = (pw_img_t){.width = PW_SCREEN_WIDTH,
                .height = 16,
                .data = eeprom_buf,
        ...
        case MS_SPLASH: {
            break;
        }  // none, we should immediately change states
        default:
