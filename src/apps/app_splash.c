...
void pw_splash_init_display(pw_state_t *s, const screen_flags_t *sf) {
    ...
    if (s->splash.inventory.caught_pokemon & INV_WALKING_POKEMON) {
        pw_screen_draw_from_eeprom(PW_SCREEN_WIDTH - 64, 0, 64, 48, PW_EEPROM_ADDR_IMG_POKEMON_LARGE_ANIMATED_FRAME2,
            PW_EEPROM_SIZE_IMG_POKEMON_LARGE_ANIMATED_FRAME, true);
    }

    pw_screen_draw_from_eeprom(
        0, PW_SCREEN_HEIGHT - 24 - 16, 32, 24, PW_EEPROM_ADDR_IMG_ROUTE_LARGE, PW_EEPROM_SIZE_IMG_ROUTE_LARGE, true);

    for (uint8_t i = 0; i < 3; i++) {
        if (s->splash.inventory.caught_pokemon & (1 << (i + 1))) {
            pw_screen_draw_from_eeprom(
                i * 8, PW_SCREEN_HEIGHT - 8, 8, 8, PW_EEPROM_ADDR_IMG_BALL, PW_EEPROM_SIZE_IMG_BALL, true);
        }
    }
    ...
    for (uint8_t i = 0; i < 3; i++) {
        if (s->splash.inventory.dowsed_items & (1 << (i + 1))) {
            pw_screen_draw_from_eeprom(
                24 + i * 8, PW_SCREEN_HEIGHT - 8, 8, 8, PW_EEPROM_ADDR_IMG_ITEM, PW_EEPROM_SIZE_IMG_ITEM, true);
        }
    }

    for (uint8_t i = 0; i < 4; i++) {
        if ((s->splash.inventory.received_bitfield & (1 << i))) {
            pw_screen_draw_from_eeprom(16 + i * 8, PW_SCREEN_HEIGHT - 16, 8, 8,
                PW_EEPROM_ADDR_IMG_CARD_SUITS + i * PW_EEPROM_SIZE_IMG_CARD_SUIT_SYMBOL,
                PW_EEPROM_SIZE_IMG_CARD_SUIT_SYMBOL, true);
        }
    }

    if (s->splash.inventory.caught_pokemon & INV_EXTRA_POKEMON) {
        pw_screen_draw_from_eeprom(
            0, PW_SCREEN_HEIGHT - 16, 8, 8, PW_EEPROM_ADDR_IMG_BALL_LIGHT, PW_EEPROM_SIZE_IMG_BALL_LIGHT, true);
    }

    if (s->splash.inventory.dowsed_items & INV_EXTRA_ITEM) {
        pw_screen_draw_from_eeprom(
            8, PW_SCREEN_HEIGHT - 16, 8, 8, PW_EEPROM_ADDR_IMG_ITEM_LIGHT, PW_EEPROM_SIZE_IMG_ITEM_LIGHT, true);
    }

    pw_screen_pos_t left_x = pw_screen_draw_integer_with_overline(
        health_data_cache.today_steps, PW_SCREEN_WIDTH, PW_SCREEN_HEIGHT - 16, PW_SCREEN_BLACK);
...
static void splash_display_pokemon(pw_state_t *s, const screen_flags_t *sf) {
    ...
    switch (s->splash.walking) {
        case IDLE: {
            pw_screen_draw_from_eeprom(
                PW_SCREEN_WIDTH - 64, 0, 64, 48, frame_addr, PW_EEPROM_SIZE_IMG_POKEMON_LARGE_ANIMATED_FRAME, true);
            break;
        }
        case WALKING: {
            pw_screen_clear_area(origin, 0, s->splash.offset, 48);
            pw_screen_draw_from_eeprom(origin + s->splash.offset, 0, 64, 48, frame_addr,
                PW_EEPROM_SIZE_IMG_POKEMON_LARGE_ANIMATED_FRAME, true);
            break;
        }
        case STROLLING: {
            ...
            pw_img_t img = {.width = 32,
                ...
                .lookup_table = {.addr = frame_addr, .use_alt = true}};
