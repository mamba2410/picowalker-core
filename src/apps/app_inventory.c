...
static void draw_cursor(pw_state_t *s, const screen_flags_t *sf) {
    uint16_t addr = (sf->frame & ANIM_FRAME_NORMAL_TIME) ? PW_EEPROM_ADDR_IMG_ARROW_DOWN_NORMAL
                                                         : PW_EEPROM_ADDR_IMG_ARROW_DOWN_OFFSET;

    pw_screen_pos_t cx = 0, cy = 0;
    get_cursor_coords(s, &cx, &cy);
    pw_screen_draw_from_eeprom(cx, cy, 8, 8, addr, PW_EEPROM_SIZE_IMG_ARROW, true);
}

static void draw_animated_sprite(pw_state_t *s, const screen_flags_t *sf) {
    uint8_t *buf = eeprom_buf;
    pw_img_t sprite = {.width = 32,
        .height = 24,
        .data = buf,
        .size = 192,  // 32*24/4
        .is_flipped = false,
    ...
    if (is_pokemon) {
        ...
        pokemon_index_t pokemon_index =
            pw_pokemon_id_to_pokemon_index(gdetailed.entries[s->inventory.current_cursor], &pokemon);
        pw_pokemon_index_to_small_sprite(
            pokemon_index, buf, (sf->frame & ANIM_FRAME_DOUBLE_TIME) >> ANIM_FRAME_DOUBLE_TIME_OFFSET, &addr);
        ...
        pw_eeprom_read(PW_EEPROM_ADDR_IMG_TREASURE_LARGE, buf, PW_EEPROM_SIZE_IMG_TREASURE_LARGE);
    }
...
static void pw_inventory_draw_screen1(pw_state_t *s, const screen_flags_t *sf) {
    ...
    pw_screen_draw_from_eeprom(
        8, 0, 80, 16, PW_EEPROM_ADDR_IMG_MENU_TITLE_INVENTORY, PW_EEPROM_SIZE_IMG_MENU_TITLE_INVENTORY, false);

    // Draw icons
    uint8_t buf_pokeball[PW_EEPROM_SIZE_IMG_BALL];
    uint8_t buf_item[PW_EEPROM_SIZE_IMG_ITEM];

    pw_img_t pokeball = {.width = 8,
        .height = 8,
        .data = buf_pokeball,
    ...
    pw_img_t item = {.width = 8,
        .height = 8,
        .data = buf_item,
        .size = PW_EEPROM_SIZE_IMG_ITEM,
        .is_flipped = false,
        .lookup_table = {.addr = PW_EEPROM_ADDR_IMG_ITEM, .use_alt = true}};
    ...
    if (gbrief.caught_pokemon & INV_EXTRA_POKEMON) {
        pw_screen_draw_from_eeprom(xs[4], yp, 8, 8, PW_EEPROM_ADDR_IMG_BALL_LIGHT, PW_EEPROM_SIZE_IMG_BALL_LIGHT, true);
    }

    // draw special item
    if (gbrief.dowsed_items & INV_EXTRA_ITEM) {
        pw_screen_draw_from_eeprom(xs[4], yi, 8, 8, PW_EEPROM_ADDR_IMG_ITEM_LIGHT, PW_EEPROM_SIZE_IMG_ITEM_LIGHT, true);
    }
...
static void pw_inventory_draw_screen2(pw_state_t *s, const screen_flags_t *sf) {
    ...
    pw_screen_draw_from_eeprom(
        0, 0, 8, 16, PW_EEPROM_ADDR_IMG_MENU_ARROW_LEFT, PW_EEPROM_SIZE_IMG_MENU_ARROW_LEFT, true);
    ...
    for (uint8_t i = 0; i < gbrief.n_peer_play_items; i++) {
        pw_screen_draw_img(&item, x0 + 8 * (i % 5), y0 + 16 * (i / 5));
    }
    ...
    if (gbrief.n_peer_play_items > 0) {
        ...
        pw_screen_draw_from_eeprom(PW_SCREEN_WIDTH - 32 - 4, PW_SCREEN_HEIGHT - 16 - 24, 32, 24,
            PW_EEPROM_ADDR_IMG_PRESENT_LARGE, PW_EEPROM_SIZE_IMG_PRESENT_LARGE, true);
