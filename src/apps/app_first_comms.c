...
void pw_first_comms_init_display(pw_state_t *s, const screen_flags_t *sf) {
    ...
    pw_img_t img = {.width = 32,
        ...
        .size = 256,
        .is_flipped = false,
        .lookup_table = {.addr = PW_FLASH_IMG_POKEWALKER, .use_alt = false}};
...
void pw_first_comms_handle_input(pw_state_t *s, const screen_flags_t *sf, pw_buttons_t b) {
    if (b == PW_BUTTON_M && pw_ir_get_comm_state() == COMM_STATE_DISCONNECTED) {
    ...
    switch (cs) {
            ...
            switch (s->comms.screen_state) {
                case FC_SUBSTATE_WAITING: {
                    ...
                    pw_img_t face = {.width = 16,
                        ...
                        .size = 32,
                        .is_flipped = false,
                        .lookup_table = {.addr = PW_FLASH_IMG_FACE_NEUTRAL, .use_alt = false}};
                    pw_flash_read(PW_FLASH_IMG_FACE_NEUTRAL, face.data);
                    pw_screen_draw_img(&img, (PW_SCREEN_WIDTH - 16) / 2, (PW_SCREEN_HEIGHT - 8) / 2);
        ...
        case COMM_STATE_SLAVE: {
            pw_img_t face = {.width = 16,
                ...
                .size = 32,
                .is_flipped = false,
                .lookup_table = {.addr = PW_FLASH_IMG_FACE_HAPPY, .use_alt = false}};
            ...
            if (sf->frame & ANIM_FRAME_NORMAL_TIME) {
                pw_img_t img = {.width = 8,
                    ...
                    .size = 16,
                    .is_flipped = false,
                    .lookup_table = {.addr = PW_FLASH_IMG_IR_ACTIVE, .use_alt = false}};
