...
void pw_switch_init_display(pw_state_t *s, const screen_flags_t *sf) {
    ...
    pw_screen_draw_from_eeprom(8, 0, 80, 16, PW_EEPROM_ADDR_TEXT_SWITCH, PW_EEPROM_SIZE_TEXT_SWITCH, false);

    for (uint8_t i = 0; i < 3; i++) {
        pw_screen_draw_from_eeprom(20 + i * (16 + 8), PW_SCREEN_HEIGHT - 32 - 8, 8, 8, addr, size, true);
    }

    pw_screen_draw_from_eeprom(20 + s->switches.cursor * (16 + 8), PW_SCREEN_HEIGHT - 32, 8, 8,
...
void pw_switch_update_display(pw_state_t *s, const screen_flags_t *sf) {
    ...
    if (sf->frame & ANIM_FRAME_DOUBLE_TIME) {
        pw_screen_draw_from_eeprom(20 + s->switches.cursor * (8 + 16), PW_SCREEN_HEIGHT - 32, 8, 8,
            PW_EEPROM_ADDR_IMG_ARROW_UP_NORMAL, PW_EEPROM_SIZE_IMG_ARROW, true);
    } else {
        pw_screen_draw_from_eeprom(20 + s->switches.cursor * (8 + 16), PW_SCREEN_HEIGHT - 32, 8, 8,
    ...
    if (s->switches.cursor != s->switches.prev_cursor) {
        pw_screen_draw_from_eeprom(0, PW_SCREEN_HEIGHT - 16, width, 16, addr, size, true);
        ...
        case SWITCHES_WRITE_INV: {
            ...
            switch (s->switches.switch_type) {
                case SWITCH_TYPE_ITEM: {
                    ...
                    pw_eeprom_write(PW_EEPROM_ADDR_OBTAINED_ITEMS, (uint8_t *)(items), PW_EEPROM_SIZE_OBTAINED_ITEMS);

                    break;
                }
                case SWITCH_TYPE_POKEMON: {
                    pokemon_summary_t caught_pokemon[3];
                    pw_eeprom_read(PW_EEPROM_ADDR_CAUGHT_POKEMON_SUMMARY, (uint8_t *)caught_pokemon,
                        PW_EEPROM_SIZE_CAUGHT_POKEMON_SUMMARY);

                    caught_pokemon[s->switches.cursor] = ri.route_pokemon[s->switches.switch_index];

                    pw_eeprom_write(PW_EEPROM_ADDR_CAUGHT_POKEMON_SUMMARY, (uint8_t *)caught_pokemon,
                        PW_EEPROM_SIZE_CAUGHT_POKEMON_SUMMARY);
                    break;
                }
            }
