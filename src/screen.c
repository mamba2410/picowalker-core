...
void pw_screen_draw_from_eeprom(pw_screen_pos_t x, pw_screen_pos_t y, pw_screen_dim_t w, pw_screen_dim_t h,
    pw_eeprom_addr_t addr, size_t len, bool use_alt) {
    pw_img_t img = {.width = w,
        .height = h,
        .data = eeprom_buf,
        .size = len,
        .is_flipped = false,
        .lookup_table = {.addr = addr, .use_alt = use_alt}};
    pw_eeprom_read(addr, eeprom_buf, len);
    pw_screen_draw_img(&img, x, y);
}

void pw_screen_draw_from_eeprom_with_text_box(pw_screen_pos_t x, pw_screen_pos_t y, pw_screen_dim_t w,
    pw_screen_dim_t h, pw_eeprom_addr_t addr, size_t len, pw_screen_color_t c) {
    pw_img_t img = {.width = w,
        .height = h,
        .data = eeprom_buf,
        .size = len,
        .is_flipped = false,
        .lookup_table = {.addr = addr, .use_alt = false}};
    pw_eeprom_read(addr, eeprom_buf, len);
...
pw_screen_pos_t pw_screen_draw_integer(uint32_t n, pw_screen_pos_t right_x, pw_screen_pos_t y) {
    ...
    do {
        ...
        pw_screen_draw_from_eeprom(
            x, y, 8, 16, PW_EEPROM_ADDR_IMG_DIGITS + PW_EEPROM_SIZE_IMG_CHAR * idx, PW_EEPROM_SIZE_IMG_CHAR, false);
    } while (m > 0);

    return x;
}

pw_screen_pos_t pw_screen_draw_integer_with_overline(
    uint32_t n, pw_screen_pos_t right_x, pw_screen_pos_t y, pw_screen_color_t c) {
    pw_screen_pos_t x = right_x;
    uint32_t m = n;
    do {
        size_t idx = m % 10;
        ...
        pw_img_t img = {.width = 8,
            ...
            .is_flipped = false,
            .lookup_table = {.addr = PW_EEPROM_ADDR_IMG_DIGITS + PW_EEPROM_SIZE_IMG_CHAR * idx, .use_alt = false}};
...
void pw_screen_draw_subtime(uint8_t n, pw_screen_pos_t x, pw_screen_pos_t y, bool draw_colon) {
    ...
    if (draw_colon) {
        x += 8;
        pw_screen_draw_from_eeprom(x, y, 8, 16, PW_EEPROM_ADDR_IMG_CHAR_COLON, PW_EEPROM_SIZE_IMG_CHAR, false);
    }
}
...
// always draws at x=0
void pw_screen_draw_message_with_text_box(
    pw_screen_pos_t y, uint8_t message_index, pw_screen_dim_t h, pw_screen_color_t c) {
    if (h != 16 && h != 32) {
        return;  // can only do 16 or 32 height messages
    }
...
void pw_screen_draw_pokemon_name_and_message(
    ...
    pw_img_t img = {.width = PW_SCREEN_WIDTH,
        ...
        .size = 2 * PW_EEPROM_SIZE_TEXT_APPEARED,
        .is_flipped = false,
        .lookup_table = {.addr = poke_addr, .use_alt = false}};
    pw_eeprom_read(poke_addr, img.data, PW_EEPROM_SIZE_TEXT_POKEMON_NAME);
    pw_eeprom_read(message_addr, img.data + PW_EEPROM_SIZE_TEXT_APPEARED, PW_EEPROM_SIZE_TEXT_APPEARED);
    for (int i = 1; i >= 0; i--) {
        for (int j = 2 * 80 - 1; j >= 0; j--) {
            img.data[i * 2 * PW_SCREEN_WIDTH + j] = img.data[i * 2 * 80 + j];
        }
    ...
    memset(img.data + PW_EEPROM_SIZE_TEXT_POKEMON_NAME, 0,
        PW_EEPROM_SIZE_TEXT_ATTACKED - PW_EEPROM_SIZE_TEXT_POKEMON_NAME);
    pw_screen_overlay_text_box(&img, PW_SCREEN_WIDTH, 32, c);
    pw_screen_draw_img(&img, 0, PW_SCREEN_HEIGHT - 32);
}
...
void pw_screen_overlay_text_box(pw_img_t *img, pw_screen_dim_t w, pw_screen_dim_t h, pw_screen_color_t c) {
    ...
    for (size_t i = 0; i < w; i++) {
        img->data[offs * img->width + 2 * i + 0] =
            (img->data[offs * img->width + 2 * i + 0] & bottom_mask) | c_upper_bottom;
        img->data[offs * img->width + 2 * i + 1] =
            (img->data[offs * img->width + 2 * i + 1] & bottom_mask) | c_lower_bottom;
    }
 ...
 */
static void overlay_img_aligned(pw_img_t *base, pw_img_t *img, pw_screen_pos_t x, pw_screen_pos_t y,
    pw_screen_dim_t visible_width, pw_screen_dim_t visible_height) {
    // Split y
    uint8_t chunk_spans = visible_height / 8;
    uint8_t x_read_offset =
        (x < 0) ? -2 * x : 0;  // Ignore first -2x bytes if its off the screen in the left x direction
 ...
 * Specifically for images with y offset aligned to 8 bytes
 */
static void overlay_img_unaligned(pw_img_t *base, pw_img_t *img, pw_screen_pos_t x, pw_screen_pos_t y,
    pw_screen_dim_t visible_width, pw_screen_dim_t visible_height) {
    // Split y
    ...
    for (int b = 0; b < 2 * visible_width; b++) {
        uint8_t top_byte = img->data[2 * img->width * (chunk - 1) + x_read_offset + b];
        uint8_t bot_byte = 0;
