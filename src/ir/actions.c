        ...
        case COMM_SUBSTATE_DETERMINE_ROLE: {
            ...
            switch (packet->cmd) {
                ...
                case CMD_ASSERT_MASTER:  // peer found us, peer requests master
                    ...
                    for (int i = 0; i < SESSION_ID_SIZE; i++) session_id_master[i] = packet->session_id_bytes[i];
...
ir_err_t pw_action_slave_perform_request(app_comms_t *comms, pw_packet_t *packet, size_t len) {
    ...
    switch (packet->cmd) {
        ...
        case CMD_WALKER_RESET_1: {
            packet->extra = EXTRA_BYTE_FROM_WALKER;
            pw_time_delay_ms(ACTION_DELAY_MS);
            pw_eeprom_reliable_read(PW_EEPROM_ADDR_UNIQUE_IDENTITY_DATA_1, PW_EEPROM_ADDR_UNIQUE_IDENTITY_DATA_2,
                packet->payload, sizeof(unique_identity_data_t));
...
ir_err_t pw_action_peer_play(app_comms_t *comms, pw_packet_t *packet, size_t max_len) {
    ...
    switch (comms->current_substate) {
        ...
        case COMM_SUBSTATE_SEND_MASTER_SPRITES: {
            ...
            err = pw_action_send_large_raw_data_from_eeprom(PW_EEPROM_ADDR_IMG_POKEMON_SMALL_ANIMATED,  // src
        ...
        case COMM_SUBSTATE_SEND_MASTER_NAME_IMAGE: {
            ...
            err = pw_action_send_large_raw_data_from_eeprom(PW_EEPROM_ADDR_TEXT_POKEMON_NAME,  // src
        ...
        case COMM_SUBSTATE_SEND_MASTER_TEAMDATA: {
            ...
            err = pw_action_send_large_raw_data_from_eeprom(PW_EEPROM_ADDR_TEAM_DATA_STRUCT,  // src
        ...
        case COMM_SUBSTATE_READ_SLAVE_SPRITES: {
            ...
            err = pw_action_read_large_raw_data_from_eeprom(PW_EEPROM_ADDR_IMG_POKEMON_SMALL_ANIMATED,  // src
                PW_EEPROM_ADDR_IMG_CURRENT_PEER_POKEMON_ANIMATED_SMALL,                                 // dst
                PW_EEPROM_SIZE_IMG_POKEMON_SMALL_ANIMATED,                                              // size
        ...
        case COMM_SUBSTATE_READ_SLAVE_NAME_IMAGE: {
            ...
            err = pw_action_read_large_raw_data_from_eeprom(PW_EEPROM_ADDR_TEXT_POKEMON_NAME,  // src
                PW_EEPROM_ADDR_TEXT_CURRENT_PEER_POKEMON_NAME,                                 // dst
                PW_EEPROM_SIZE_TEXT_POKEMON_NAME,                                              // size
                read_size, &(comms->advertising_attempts), packet, max_len);
...
void pw_ir_start_walk() {
    ...
    buf[0] = 0xa5;
    int res = pw_eeprom_reliable_write(PW_EEPROM_ADDR_COPY_MARKER_1, PW_EEPROM_ADDR_COPY_MARKER_2, buf, 1);
    if (res < 0) {
    ...
    buf[0] = 0x00;
    res = pw_eeprom_reliable_write(PW_EEPROM_ADDR_COPY_MARKER_1, PW_EEPROM_ADDR_COPY_MARKER_2, buf, 1);
    if (res < 0) {
